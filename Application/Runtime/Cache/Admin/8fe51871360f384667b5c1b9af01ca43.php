                            ...
                            <div class="span12">
                                ...
                                <div class="portlet box blue tabbable">
                                    <div class="portlet-title">
                                        <div class="caption"><i class="icon-reorder"></i><?php echo ($name); ?></div>
                                    </div>
                                                    ...
                                                    <div class="portlet">
                                                        <div class="portlet-body">
                                                            ...
                                                            <table class="table table-striped table-hover table-bordered" id="sample_editable_1">
                                                                ...
                                                                <tbody>
                                                                <?php if($state == 1 ): if(is_array($notice_list)): $i = 0; $__LIST__ = $notice_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$value): $mod = ($i % 2 );++$i;?><tr class="">
                                                                ...
                                                                <?php elseif($state == 2): ?>
                                                                <?php if(is_array($enterprise_list)): $i = 0; $__LIST__ = $enterprise_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><tr class="">
                                                                ...
                                                                <?php elseif($state == 3): ?>
                                                                <?php if(is_array($falsh_list)): $i = 0; $__LIST__ = $falsh_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr class="">
                                                                    <td><a href="javascript:;" class="contentAlert"><?php echo ($v["title"]); ?></a></td>
                                                                    <td><?php echo ($v["description"]); ?></td>
                                                                    <td><?php echo ($v["from"]); ?></td>
                                                                    <td>
                                                                    	<input type="hidden" class="id" name="field＿name" value="<?php echo ($v["id"]); ?>"> 
                                                                   		<input type="hidden" class="state" name="field＿name" value="<?php echo ($state); ?>">
                                                                        <a class="y_clolor_red" href="javascript:;" style="display: inline-block">删除</a>
                                                                        <a class="y_clolor_yellow" href="javascript:;" style="display: inline-block">修改</a>
                                                                        <a class="y_clolor_green" href="javascript:;" style="display: inline-block">推送</a>
                                                                ...
                                                                <?php else: ?>
                                                                <?php if(is_array($media_list)): $i = 0; $__LIST__ = $media_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="">
                                                                    <td><a href="javascript:;" class="contentAlert"><?php echo ($vo["title"]); ?></a></td>
                                                                    <td><?php echo ($vo["description"]); ?></td>
                                                                    <td><?php echo ($vo["from"]); ?></td>
                                                                    <td>
                                                                    	<input type="hidden" class="id" name="field＿name" value="<?php echo ($vo["id"]); ?>"> 
                                                                   		<input type="hidden" class="state" name="field＿name" value="<?php echo ($state); ?>"> 
                                                                        <a class="y_clolor_red" href="javascript:;" style="display: inline-block">删除</a>
                                                                        <a class="y_clolor_yellow" href="javascript:;" style="display: inline-block">修改</a>
                                                                        <a class="y_clolor_green" href="javascript:;" style="display: inline-block">推送</a>
...
<script>
	...
	$('.contentAlert').click(function(){
		var id = $(this).parent().next().next().next().children('.id').val();
		var state = $(this).parent().next().next().next().children('.state').val();
		$.post("<?php echo U('Content/newsAlert');?>",{'id':id,'state':state}, function (data) {
			layui.use('layer',function(){
				var layer = layui.layer;
				layer.open({
					  type: 1,
					  //skin: 'layui-layer-rim', //加上边框
					  area: ['1000px', '780px'], //宽高
					  content: data
					});
			})
        });
	});
	$('.y_clolor_yellow').click(function(){
		var id = $(this).prev().prev().prev().val();
		var state = $(this).prev().prev().val();
		$.post("<?php echo U('Content/editNew');?>",{'id':id,'state':state}, function (data) {
			layui.use('layer',function(){
				var layer = layui.layer;
				layer.open({
					  type: 1,
	...
	$('.y_clolor_green').click(function(){
		var id = $(this).prev().prev().prev().prev().val();
		var table=$("[name='table']").val(); 
		//alert(id);
		$.post("<?php echo U('Content/tuisong');?>",{'id':id,'table':table}, function (data) {
			layui.use('layer',function(){
				var layer = layui.layer;
				if(data){
					layer.alert('推送成功');
	...
	$('.y_btn').click(function(){
		...
		var table=$("[name='table']").val(); 
		$.post("<?php echo U('Content/serchAll');?>",{'title':title,'table':table}, function (data) {
			//alert(data);
			$('tbody').html(data);
		});
    });
